...
import logo from '../../assets/medal.png'; // with import
...
class Deposits extends Component {
  ...
  async componentDidMount() {
    axios.get(`https://api.github.com/users/jprcorreia`)
    .then(res => {
      ...
      <>
        <Grid container spacing={3}>
          ...
          <img src={logo} alt="Logo" style={{width: 35, height: 35, marginLeft:200, marginBottom: -20}}/>
            <h3 style={{marginLeft:-10, marginBottom: -10}}>1º lugar</h3>
            <h1 style={{marginLeft:40}}>{this.state.username}</h1>
            <h3 style={{ marginTop: -10}}>36 pontos</h3>
          </Grid>
        </Grid>
      </>
    );
